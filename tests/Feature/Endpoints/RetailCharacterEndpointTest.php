...
class RetailCharacterEndpointTest extends EndpointIntegrationTestCase
{
    ...
    public function test_retail_endpoint_includes_titles(array $fixture, string $slot): void
    {
        ...
        $response->assertJsonIsArray('data.titles');
        $titles = $response->json('data.titles');
        foreach ($titles as $i => $title) {
            $this->assertArrayHasKey('id', $title, "titles[{$i}] missing id");
            $this->assertArrayHasKey('name_male', $title, "titles[{$i}] missing name_male");
            $this->assertArrayHasKey('name_female', $title, "titles[{$i}] missing name_female");
            $this->assertIsInt($title['id']);
            $this->assertIsString($title['name_male']);
            $this->assertIsString($title['name_female']);
        }

        $this->assertSame('fresh', $response->json('meta.freshness.titles'));
    }
    ...
    public function test_titles_response_shape_after_normalization(): void
    {
        ...
        $response->assertJsonPath('data.active_title_id', 414);
        $response->assertJsonPath('data.titles.0.id', 414);
        $response->assertJsonPath('data.titles.0.name_male', '{name}, Lord of the Bears');
        $response->assertJsonPath('data.titles.0.name_female', '{name}, Lady of the Bears');
    }

    public function test_active_title_id_null_when_none_equipped(): void
    {
        GameDataTitle::create([
            'id' => 88888,
            'name_male' => 'Orphan Title',
            'name_female' => 'Orphan Title',
        ]);
        ...
        $this->assertNull($response->json('data.active_title_id'));
