...
- **Delete-missing semantics.** `character_pvp_brackets`, `character_professions`, `raid_encounter_kills`, `character_titles` all upsert then delete rows not present in the latest response, inside the slice's `DB::transaction`. An empty response (or 404) wipes that slice for the character — required behavior so a dropped profession, unplayed bracket, reset lockout, or untrained title disappears.
