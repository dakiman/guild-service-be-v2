...
- **Ranks (nightly):** `ranks:materialize` (04:00) rebuilds `character_ranks` + `realm_run_boards` from season-fresh ratings (`rating_synced_at >= season.started_at`, endgame, rating > 0) and stamps `ranks:computed_at`. `/leaderboards/*` and the character `rank` block read only that table; nothing computes ranks inline. `ratings:refresh` (06:30, `BLIZZARD_RATING_REFRESH_ENABLED`, cap `BLIZZARD_RATING_REFRESH_DAILY_CAP`) Shallow-resyncs stale rated characters, ladder members first. After a rollover ranks are empty until the lane refills them.
