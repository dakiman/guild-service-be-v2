...
- **Stats slice.** `stats` JSONB column on `characters` carries the Blizzard `/statistics` payload (envelope keys `_links` and `character` stripped). Note: the trailing path segment is `statistics`, NOT `character-stats` — the latter 404s. Tracks freshness via `stats_synced_at`. A 404 from Blizzard writes `stats = null` and updates `stats_synced_at` (delete-missing semantics). Always-on after Plan 5 cleanup.
...
- **Game-data achievements resolver (Plan 5).** Two tables: `game_data_achievement_categories` (~few hundred rows; `parent_id` self-reference is **not FK-constrained** because Blizzard's category index returns children before parents — same pattern as `game_data_factions.parent_faction_id`) and `game_data_achievements` (~40k rows; `name`, `description`, `category_id` FK, `points`, `is_account_wide`). Both populated by `php artisan blizzard:sync-game-data achievements` (categories phase first as the FK target for achievements; achievements phase upserts in chunks of 500 per `DB::transaction` because a single transaction over ~40k rows holds locks too long). Sync command extends `BlizzardGameDataClient` with four `static-{region}` methods (`getAchievement{Index,}`, `getAchievementCategory{Index,}`). The tables exist purely as join targets — there is no eager-load on `CharacterResource` and no client-side catalog endpoint. See **Character achievements endpoint** below for how the FE consumes them.
- **Game-data PvE resolver (Plan A — PvE tab redesign).** Four new tables: `game_data_raid_instances` (PK = Blizzard journal-instance id, FK to `game_data_expansions`), `game_data_raid_encounters` (PK = journal-encounter id, FK to raid_instances with cascade delete; carries `creature_display_id` + `portrait_url`), `game_data_mythic_keystone_dungeons` (PK = mythic-keystone dungeon id; `journal_instance_id` is a soft join key, **not** FK-constrained, since older-expansion dungeons may reference an instance we did not sync), `game_data_keystone_affixes` (PK = affix id; `icon_url`). Populated by `php artisan blizzard:sync-game-data pve` — extends the same command that already powers the Plan-5 resolvers, scheduled weekly with the rest. Sync sequence: (1) `/data/wow/journal-instance/index` + per-id detail/media → upsert raid instances + their encounter rosters (encounter detail + creature-display media give the boss portrait), (2) `/data/wow/mythic-keystone/season/index` resolves the current season id, then `/data/wow/mythic-keystone/season/{id}.dungeons` drives a per-id `/mythic-keystone/dungeon/{id}` fan-out (dungeon endpoints live in **`dynamic-{region}`** namespace, unlike the journal-instance family which is `static-{region}`), (3) `/data/wow/keystone-affix/index` + per-id detail + media. Per-instance `DB::transaction` (one per raid wrapping its encounter fan-out) — deliberate departure from the Plan-5 single-mega-transaction pattern, since holding one transaction across ~30 raids × ~8 encounters × 2-3 Blizzard calls each would exceed the rate-limit budget for a single tx. Per-id failure tolerance: a 404 or thrown error on any single instance/encounter/dungeon is logged and skipped without aborting the rest of the sync. Dungeon `media_url` is **always null** today — Blizzard does not expose a media endpoint for `/data/wow/mythic-keystone/dungeon/{id}`; column is reserved. Two new public endpoints (no auth) on a new `GameDataController`: `GET /api/v1/game-data/raid-instances?expansion=current|all` (default `current` scopes to `game_data_expansions.display_order=1`; `all` returns every expansion; both eager-load `encounters`) and `GET /api/v1/game-data/mythic-keystone-dungeons?season=current` (returns dungeons + the season's affixes piggybacked into the same payload — keyed by id, ~12-16 affix rows). Both endpoints emit `Cache-Control: max-age=3600, public` (Symfony normalizes the directive ordering alphabetically — semantically identical to the spec's `public, max-age=3600`) per spec §2.6 — FE caches via TanStack Query with `staleTime: Infinity`. No feature flag — endpoints serve whatever is in the tables, and an empty table yields `data: []` rather than an error.
- **Character achievements endpoint.** `GET /api/v1/characters/{region}/{realm}/{name}/achievements` (`CharacterAchievementsController::index`) returns cursor-paginated, joined rows: `{achievement_id, completed_timestamp, name, category_name}` plus `meta.{total, per_page, next_cursor}`. Default per_page=100, max=200. Ordering is `completed_timestamp DESC NULLS LAST` with an `achievement_id DESC` tiebreaker; the cursor is a base64url JSON `{ts, id}` so NULL-timestamp rows paginate correctly through their own tail. Default filters out the `Feats of Strength` category; `?include_feats=1` re-includes them. The character payload itself no longer carries achievements — `CharacterController` does not eager-load the relation, and `CharacterResource` does not emit it. The FE consumes this via `useInfiniteQuery` in `AchievementsList.vue` and prefetches the next page when the virtualizer is within ~200px of the end. Missing-row fallback (achievement_id present on the character but absent from `game_data_achievements`): the join returns `name=null`/`category_name=null` and the FE renders `Achievement {id}`. No feature flag.
