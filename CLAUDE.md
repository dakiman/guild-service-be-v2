...
- **Architecture.** Single `RaiderIOClient` (Laravel `Http::` wrapper + Redis token-bucket throttle at 175/min, ~12% under raider.io's documented 200/min public ceiling per swagger), one `RaiderIOSeeder` orchestrator with `seedGuilds()` and `seedRuns()` methods, one artisan command (`raiderio:seed`). Phase 3 (per-spec character leaderboard) is **cancelled** — verified via raider.io/swagger.json that no public per-character ranking endpoint exists; only the 35 documented paths covering guild rankings, run rankings, and single-entity profiles. Reuses existing `SyncGuildData` / `SyncCharacterData` jobs end-to-end — seeder dispatches and forgets.
...
- **Phase 3 (cancelled): Characters.** Originally specced as top-characters-per-spec from a `/mythic-plus/leaderboards` endpoint, but raider.io/swagger.json confirms no such public endpoint exists — there is no per-character or per-spec ranking API, only the guild-rankings (Phase 1) and run-rankings (Phase 2) endpoints. To broaden coverage, bump `RAIDERIO_SEED_GUILDS_PER_REGION` and `RAIDERIO_SEED_RUNS_PAGES_PER_REGION` instead.
- **Dedupe model.** Guilds: existing `Guild::isRosterStale()` (config/blizzard.php threshold). Runs: `seeded_runs` ledger keyed on `keystone_run_id` (no TTL — runs are immutable; `seeded_at` indexed for future cleanup). Characters (run members): `Character.updated_at` against `RAIDERIO_SEED_CHAR_TTL` (default 12h). `--force` flag bypasses TTL gates only — does NOT bypass the `seeded_runs` ledger (truncate the table to re-seed historical runs).
- **Roster fan-out (per-job opt-in, not global default).** `SyncGuildRoster` dispatches per-member `SyncCharacterData::Full` *only when* (a) it received `forceFanout: true` from `SyncGuildData` (which itself gets `forceRosterFanout: true` from the seeder), OR (b) the global `RAIDERIO_DISPATCH_ROSTER_CHAR_SYNCS` config flag is on (default `false`). This default-off setup means routine `ProactiveSyncGuilds` and user-initiated `SyncGuildData` do NOT cascade Full per-member — only the seeder does. TTL gate reads `Character.updated_at` against `RAIDERIO_SEED_CHAR_TTL` (default 12h). Members above `BLIZZARD_MIN_LEVEL_FOR_LOOKUP` only. **Two pre-existing bugs were fixed during phase 1 to make this path actually run at all:** (1) `SyncGuildData` previously gated the `SyncGuildRoster::dispatch` call behind `if ($guild->isRosterStale())` *immediately after* setting `roster_synced_at = now()` — the gate was always false, so `SyncGuildRoster` never fired in production. The gate was removed. (2) `SyncGuildRoster::handle` previously dispatched member syncs via `Bus::batch(SyncCharacterData[])`, but `SyncCharacterData` does not use the `Batchable` trait — every batch dispatch threw `RuntimeException`. The batch was replaced with a `foreach` of individual `SyncCharacterData::dispatch` calls. Both bugs were latent because they cancelled each other out.
...
`RAIDERIO_BASE_URL`, `RAIDERIO_ACCESS_KEY` (optional API key from raider.io/settings/apps; unlocks higher rates above the 200/min public ceiling), `RAIDERIO_RATE_PER_MINUTE` (default 175 — under raider.io's documented 200/min public limit), `RAIDERIO_SEED_REGIONS` (default `eu,us`), `RAIDERIO_SEED_SEASON` (default `season-mn-1`), `RAIDERIO_SEED_GUILDS_PER_REGION` (default 10),
`RAIDERIO_SEED_RUNS_PAGES_PER_REGION` (default 5; one page = 20 runs = 100 character dispatches),
`RAIDERIO_SEED_CHAR_TTL` (default 43200 = 12h), `RAIDERIO_SEED_TEAMMATE_CRAWL_ENABLED` (default false), `RAIDERIO_DISPATCH_ROSTER_CHAR_SYNCS` (default **false** — flips on roster Full fan-out for *all* `SyncGuildRoster` runs, not just seed-originated; seeder uses a per-job override instead).
