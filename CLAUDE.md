...
- **Roster fan-out.** `SyncGuildRoster` adds a per-member `SyncCharacterData::Full` dispatch loop alongside the per-member Shallow dispatches, gated by `RAIDERIO_DISPATCH_ROSTER_CHAR_SYNCS` (default true). TTL gate reads `Character.updated_at` against `RAIDERIO_SEED_CHAR_TTL` (default 12h = 43200s). Members above `BLIZZARD_MIN_LEVEL_FOR_LOOKUP` only. **Two pre-existing bugs were fixed during phase 1 to make this work:** (1) `SyncGuildData` previously gated the `SyncGuildRoster::dispatch` call behind `if ($guild->isRosterStale())` *immediately after* setting `roster_synced_at = now()` — the gate was always false, so `SyncGuildRoster` never fired in production. The gate was removed; dispatch is now unconditional. (2) `SyncGuildRoster::handle` previously dispatched member syncs via `Bus::batch(SyncCharacterData[])`, but `SyncCharacterData` does not use the `Batchable` trait — every batch dispatch threw `RuntimeException`. The batch was replaced with a `foreach` of individual `SyncCharacterData::dispatch` calls. Both bugs were latent because they cancelled each other out (gate prevented the batch from ever running).
...
`RAIDERIO_SEED_CHAR_TTL` (default 43200 = 12h), `RAIDERIO_SEED_TEAMMATE_CRAWL_ENABLED` (default false), `RAIDERIO_SEED_CHUNK` (default 50), `RAIDERIO_DISPATCH_ROSTER_CHAR_SYNCS` (default true).
